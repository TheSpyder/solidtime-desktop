...
// events and decides whether the user is present. The idle monitor and the
// workday monitor subscribe as policy consumers; the input poll runs while at
// least one subscriber is registered, so either feature being enabled is
// enough to keep the signal alive.
//
// Gap rule (shared by both consumers): a pause — keyboard idle, sleep, or
...
/**
 * Current idle threshold in seconds — the shared gap rule for both
 * consumers: a pause shorter than this counts as continuous activity.
 */
